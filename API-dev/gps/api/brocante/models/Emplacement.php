...
/**
     ...
     * @return void
     */
    public function lireUnByGPS(){
        // On écrit la requête
        // JPP a metre a jour Y 0:41:54
        $sql = "SELECT * FROM " . $this->table . "  WHERE lat = ? AND lon = ?";
        //$sql = "SELECT * FROM " . $this->table . "  WHERE lat=:lat AND lon=:lon;";

        // On prépare la requête
        $query = $this->connexion->prepare( $sql );

        // On attache lat et lon
        $query->bindParam(1, $this->lat);
        $query->bindParam(2, $this->lon);
